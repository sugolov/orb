// Orchestrator panel. Grows out of the orb being entered, collapses
// back into it on close / transition. Two-column layout: Memory (left)
// and Chat (right). Header has Back, breadcrumb, status, pin/merge/
// promote actions, and a destructive ✕.
...
interface PanelProps {
  ...
  /** Map of orb_id → memory items owned by that orb. We compute the
   *  inherited chain locally by walking parent_id for the same reason
   *  the backend does — the tree IS the context graph. */
  memory: Map<string, MemoryItem[]>;
  transitionOrigin: ScreenPos;
  phase: Phase;
  onClose: () => void;
  /** Click on a suborb pill in chat → opens a floating chat window
   *  for that suborb (no longer recurses into its orchestrator).
   *  screenPos is the click position so the window opens centered on
